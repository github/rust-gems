...
impl<'a, C: GeoConfig<Diff>> GeoDiffMetric<'a, C> {
    ...
    /// Prefetches up to one 4 KiB page of the filter data first read by
    /// [`Self::symmetric_diff_size`] into the L2 cache.
    ///
    /// Hints are issued for every 64-byte cache line overlapping the available dense lower-bit
    /// storage, or the sparse upper-bit storage when no dense blocks are present. The walk is
    /// bounded by the backing slice, so short filters are not prefetched beyond their safely
    /// addressable storage. Like [`Self::prefetch_l1`], this is a CPU cache hint only and performs
    /// no I/O.
    #[inline]
    pub fn prefetch_l2_page(&self) {
        let blocks = self.filter.lsb.blocks();
        if blocks.is_empty() {
            prefetch_l2_page_data(self.filter.msb.as_ref());
        } else {
            prefetch_l2_page_data(blocks);
        }
    }
}
...
#[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
const DATA_PAGE_BYTES: usize = 4096;

#[cfg(any(target_arch = "x86_64", target_arch = "aarch64"))]
#[inline]
fn prefetch_l2_page_data<T>(data: &[T]) {
    let len = std::mem::size_of_val(data).min(DATA_PAGE_BYTES);
    if len == 0 {
        return;
    }
    ...
    // Start at the next cache-line boundary so an unaligned slice still touches every line that
    // overlaps its first page of data.
    let alignment = ptr.align_offset(CACHE_LINE_BYTES);
    let mut offset = if alignment == 0 {
        CACHE_LINE_BYTES
    } else {
        alignment
    };
    while offset < len {
        // SAFETY: `offset < len`, and `len` is bounded by the size of `data`, so this pointer
        // remains within the same live backing slice.
        prefetch_l2(unsafe { ptr.add(offset) });
...
#[inline]
fn prefetch_l2_page_data<T>(_data: &[T]) {}
...
fn prefetch_l2(ptr: *const u8) {
    // SAFETY: the caller provides a pointer within a live backing slice. `_mm_prefetch` only
    // issues a cache hint and does not expose or retain the pointer.
    unsafe {
        std::arch::x86_64::_mm_prefetch::<{ std::arch::x86_64::_MM_HINT_T1 }>(ptr.cast());
    }
}
...
#[inline]
fn prefetch_l2(ptr: *const u8) {
    // SAFETY: the caller provides a pointer within a live backing slice. `prfm` only issues a
    // cache hint and does not expose or retain the pointer.
    unsafe {
        std::arch::asm!("prfm pldl2keep, [{ptr}]", ptr = in(reg) ptr, options(readonly, nostack));
    }
}
